...
all = Join[First/@usints,GetAllInts[FileNames["ibps/ints_SR212m3_*.mx"]], First/@exints];
target = Get["target"];
If[target === 0,
...
If[!ContainsAll[all,target],
  ...
  Put[{{},target,MappedSectors[SR212m3],UniqueSectors[SR212m3],Table[{ii},{ii,Length@target}]}, FileNameJoin@{resdir, "datainfo"}];
...
eqs=(Import[#]/.{s -> 3/2, msq -> 1, "d" -> 1/7})&/@eqsfiles;
nids=Length/@eqs;
...
Put[{nonmis,mis,MappedSectors[SR212m3],UniqueSectors[SR212m3],ConstantArray[{},Length[nonmis]]}, FileNameJoin@{resdir, "datainfo"}];
Abort[]];
...
Put[{nonmis,mis,MappedSectors[SR212m3],UniqueSectors[SR212m3],sparseinfo}, FileNameJoin@{resdir, "datainfo"}];
Put[usints,FileNameJoin@{resdir, "usints"}];
Put[exints,FileNameJoin@{resdir,"exints"}];
