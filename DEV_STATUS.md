...
> 更新于 2026-05-15，覆盖 2026-05-05 9:00 至今的改动。
...
8. [近期进展 (2026-05-13 — 2026-05-15)](#8-近期进展-2026-05-13--2026-05-15)
...
| Box | 1 | 4 | 15 | ✅ | ✅ |
| Penta1L | 1 | 5 | — | ✅ | ✅ |
| Penta1L-5m | 1 | 5 | — | ✅ | ✅ (C++ + MMA) |
| SR212 | 2 | 5 | 6 | ✅ | ✅ |
| SR212-3m | 2 | 5 | 8 | ✅ | ✅ |
| SR212-5m | 2 | 5 | 45 | ✅ | ✅ |
| NP222var | 2 | 7 | — | ✅ | ✅ |
| TB123 | 2 | 7 | 37 | ✅ | ✅ (IncrementalNullspace 优化后 ~300s) |
| TB123m | 2 | 7 | — | ✅ | ⏳ 待 region solve |
| DB313 | 2 | 7 | — | ✅ | ⏳ 待 region solve |
| DB313-4m | 2 | 7 | — | ⏳ | 新建 JSON |
| NP222 | 2 | 7 | — | ✅ | ✅ (topSector 已修复) |
| NP322 | 2 | 8 | — | ✅ | ⏳ 待 region solve |
| NP322m | 2 | 8 | — | ✅ | 新建 JSON |
...
- [ ] TB123 Groebner 基性能（当前 IncrementalNullspace 优化后 ~300s，仍偏慢）
- [ ] DB313 / DB313-4m / TB123m / NP322 / NP322m region solve
...
./test_relationFF bub 4 1 2 2 --output /tmp/out  # 输出到指定目录 (commit ebd23e3)
```
...
| `DEV_STATUS.md` | 本文件 |
| `tools/generating_cone.cpp` | SymbolicRule CLI 入口（124 行，完整实现） |
| `tools/family_scheduler.py` | Region solve 任务调度器 |
...
```
project/
├── families/              # 27 个 JSON family 定义（含 1L/2L/3L + 变体）
├── include/               # 11 个管线头文件（header-only）+ SymbolicRule / RelationLoader
├── tools/                 # family_generate CLI + generating_cone + family_scheduler.py
├── tests/                 # 15 个测试源文件（含 test_generating_cone）
...
├── build/                 # CMake 构建产物
├── workspace/             # 25 个任务工作区（C001–C025）
│   ├── C001–C025/         # 各任务的 region solve / relation test / verification
│   ├── shared/            # 共享数据（mma-refs, VerifyUtility, bub00-11, Penta1L-5m, timing）
│   └── checkpoint/        # 会话存档
├── cache/                 # 调度器缓存（DB313_scheduled 等）
├── verify/
│   ├── FamilyDatabase/    # 统一 Family 定义 (MMA) + README
│   ├── VerifyUtility/     # MMA 参考管线脚本
│   │   └── cache/         # Kira 验证缓存（bub00/SR212/Tri）
│   ├── docs/              # 验证结果文档（3 个）
│   └── <family>/          # 14 个 family 的 MMA 参考 .bin + 元数据
└── DEV_STATUS.md          # 本文件
```
```
...
### 6.4 IncrementDetector

`commit e90e9a2` — 新增基于 rank 的 incre 自动检测机制：
- 在 LayerRecursion 中自动推断最优 `incre` 参数
- 避免手动设置 `incre` 导致的过估计或欠估计
...
- [x] `tools/generating_cone.cpp` — CLI 入口（124 行，完整实现含 usage/verbose/时序/验证）
- [x] `CMakeLists.txt` 已添加 `generating_cone` 和 `test_generating_cone` 构建目标
...
## 8. 近期进展 (2026-05-13 — 2026-05-15)

### 8.1 项目重组

`commit 256ddcc` — 大规模重组项目结构为 `src/ workspace/ docs/` 布局：
- `workspace/` 新增 25 个任务工作区（C001–C025），管理 region solve / relation test / verification
- 每个任务工作区包含独立的输出、日志、缓存目录
- `workspace/shared/` 存放共享数据（mma-refs, VerifyUtility, timing）
- 原 `mma/`、`checkpoint/` 内容归入 `workspace/shared/` 和 `workspace/checkpoint/`

### 8.2 问题修复

| 问题 | 状态变化 | 详情 |
|------|----------|------|
| NP222 topSector 错误 | ✅ 已修复 | `86f8214`: 1111111 → 1111110（6-prop max） |
| SymbolicRule CLI | ✅ 已实现 | `tools/generating_cone.cpp` (124行)，CMake 构建目标已添加 |
| JSON config 覆盖 | ✅ 已完成 | 27 个 JSON family（原 9 个），含 DB313/NP322/NP322m/TB123m/3-loop families |
| Penta1L-5m | ✅ 完成 | C++ + MMA 双路线均生成 region data，有 AllRelation 输出 |

### 8.3 新增 Family

`commit 256ddcc` 一次性新增 18 个 JSON family config：

| 类型 | Families |
|------|----------|
| 2L (原有补全) | DB313, NP222var, NP322, NP322m, TB123m, Penta1L, Penta1L-5m |
| 2L 变体 | SR212-3m, SR212-5m |
| 3L 新增 | BN3L, BN3L1P, BN3L1Pm, DP2L5P, DP323, HB2L5P, LA3L4Pm, LB3L4Pm, PB2L5P |
| 其他 | DB313-4m |
...
| 工具 | 文件 | 功能 |
|------|------|------|
| `generating_cone` | `tools/generating_cone.cpp` | SymbolicRule 三锥计算 CLI |
| `family_scheduler.py` | `tools/family_scheduler.py` | Region solve 任务自动调度 |

### 8.5 任务工作区激活的任务

| 工作区 | Family | 类型 | 状态 |
|--------|--------|------|------|
| C018 | NP222 | C++ region solve | ✅ 完成 |
| C019 | TB123m | C++ region solve | 运行中 |
| C020 | SR212 | C++ region solve | 运行中 |
| C021 | SR212-3m | C++ region solve | 运行中 |
| C022–C025 | Penta1L-5m | region solve + relation test | 运行中 |
...
| 问题 | 影响 | 状态 |
|------|------|------|
| buildABEquations nIdx 过滤 | 非完整 sector 缺失跨传播子耦合 | ✅ 已修复：commit 518f5ef，使用 FULL ideal |
| TB123 Singular 性能 | Groebner 基 ~300s | 可运行，待优化 |
...
86f8214  fix: NP222 topSector 1111111 → 1111110 (6-prop max)
70cba13  create C018: NP222 region-solve (cpp-scheduled)
fe55cc3  move scheduler cache to task workspace
cffd848  reset C013: switch from C++-unscheduled to cpp-scheduled
d315213  reset C012: switch from MMA to C++-scheduled pipeline
7803744  sync C017: recreate workspace dir with status.json
ebd23e3  feat: test_relationFF --output <dir>; output to task workspace
55163a3  move bub00 test outputs to C006 workspace
b35399a  move mma-refs/ → workspace/shared/mma-refs/
5a554a8  cleanup: remove DP323_recompute working directory
8e5b178  merge mma/ into workspace/shared/
95d2d79  cleanup: remove superseded MMA verify scripts, organize mma/
6c0c609  cleanup: move test artifacts from build/ and data/ to workspace/shared/
d82c9ca  fix: restore cache/DB313_scheduled, update run_scheduled.wl paths
256ddcc  restructure: reorganize project into src/ workspace/ docs/ layout with pipeline stages
```
