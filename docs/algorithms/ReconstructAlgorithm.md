...
**管线位置**: Stage 3 — 读取 LayerRecursion 的展开系数，重建 IBP 约化关系，输出 `AllRelations_*.m`。`SymbolicRuleAlgorithm.md`（Stage 4）在此基础上验证完备性。

### 输入/输出接口

| | 类型 | 说明 |
|---|---|---|
| **输入** | **内存接口** | C++ same-process: `seriesCoefficient<T>` 数组 → `reconstructAllRelations()` 直接调用<br>MMA: `hexpnList`（展开系数表）、`aregList`（区域规范）、`topsec`、`vList` |
| **处理** | 内存中 | 逐 seed 级别生成 ansatz → 计算基幂 → 建立方程 → 求解 → 稳定性检测 |
| **输出** | **文件接口** | `AllRelations_<family>_k<order>.m` — MMA 格式列表文件<br>每行形如: `{"g[ν-α] 系数" -> c_i(ν), ...}`，表示 $\sum_i c_i(\nu)\,g(\nu-\alpha_i)=0$<br>C++ `test_relationFF` 自动写入 `relations/` 目录 |
| **下游** | 文件接口 | `SymbolicRuleAlgorithm.md` — 读取 `AllRelations_*.m` 作为输入 |
...
| `mma/ReconstructReductionRelation.wl` | MMA 纯实现 | `ReconstructReductionRelation[lev, deg, order, hexpnList, aregList, topsec, vList]` |
...
---
## Appendix A: ReconstructReductionRelation.wl MMA Package API Reference

### A.1 概述

`ReconstructReductionRelation.wl` 是用于从大型指标展开（Large Index Expansion）中重建 IBP 约化关系的 Mathematica 包。支持多区域、多解以及代数扩张域上的计算。
...
| `order` | Integer | 展开阶数 |
| `hexpnList` | List | 展开系数表 |
| `aregList` | List | 区域规范列表（Association，含代数扩张数据） |
| `topsec` | List | 顶层 sector 规范 |
| `vList` | List | 变量列表 |
| `opts` | Options | 算法选项 |
...
| 选项 | 默认值 | 说明 |
|------|--------|------|
| `Modulus` | `0` | 模数（0=精确/Q, p=F_p） |
| `Verbose` | `False` | 调试输出 |
| `"AnsatzMode"` | `0` | 0=金字塔, 1=点金字塔, 2=星形 |
...
├── Package Header — 导出函数 + Usage 定义
├── Configuration and Global State — $RRRVerbose, WithIndent, PrintV, MyTimer
├── Options — 选项定义
├── Utility Functions — FirstNonZero, FFReduce, GenerateSeeds, CircledMinus, ...
├── Module 1: GenerateAnsatz[mode, rank, maxDeg, limitSector, nVars, vlist]
├── Module 2: Basis Power Computation — MonomialRulesPower, ComputeBasisPower
├── Module 3: Expansion Table Management — UpdateExpansionTable
├── Module 4: Variable Management — RemoveSolvedVariables, InitializeVariables, ...
├── Module 5: Equation Solver Core — SolveDegreeEquations, SetupEquationsAll/Single
└── Main Function — ReconstructReductionRelation 主流程
```
...
```
1. 初始化 — 设置选项、变量、vFixed
2. GenerateAnsatz — 生成各级别 ansatz
3. ComputeBasisPower — 计算每区域的 A^(-alpha)
4. 初始化展开表 — 创建 expnTable
5. 主循环：遍历 seed 级别
   ├─ UpdateExpansionTable
   └─ 循环：遍历 deg
       ├─ RemoveSolvedVariables + InitializeVariables
       ├─ SetupCoefficientTable
       ├─ SolveDegreeEquations (All / RegionByRegion)
...
vList = {v1, v2};
hexpnList = {{{<|{0,0} -> 1, {1,0} -> v1|>, <|{0,0} -> 0, {1,0} -> 1|>}}};
aregList = {<|"VarDep" -> {A1, A2}, "VarRule" -> {A[1]->1, A[2]->1},
   "VarIndep" -> {}, "MonomialBasis" -> {1},
   "MonomialBasisIndex" -> {{}},
   "MonomialBasisMatrix" -> <|{} -> {{1}}|>,
   "LimitSector" -> {1, 1}|>};
result = ReconstructReductionRelation[2, 1, 3, hexpnList, aregList, {1, 1}, vList, Verbose -> True];
```

## 相关文档

- `RegionSolverAlgorithm.md` — Region Solver 管线上游（生成重构算法输入 .bin 文件）
- `LayerRecursion_Algorithm.md` — Layer Recursion 展开算法（管线上游）
- `SymbolicRuleAlgorithm.md` — 符号规则生成（含 RemoveSolvedVariables 扩展）
- `RelationSolver_ComponentGuide.md` — C++ 关系求解器组件完整参考
- `RelationSolver_Documentation_Hub.md` — RelationSolver 文档导航中心
- `Benchmark_Results.md` — 重构算法性能数据
- `../verify/Test-Relation.md` — 关系输出验证工作流
- `../verify/Test-Expand.md` — 展开一致性验证（管线上游）

### A.8 版本信息

- 文件：`ReconstructReductionRelation.wl`
- 行数：约 1004 行
- 导出：`ReconstructReductionRelation`, `GenerateAnsatz`, `ComputeBasisPower`
