(* run_scheduled.wl -- MMA ScheduledRegionSolve for Box (4-props, 15 sectors) *)
...
Print["========================================"];
Print["ScheduledRegionSolve Box (MMA)"];
...
scheduleConfig = <|
  ...
  "MaxRetriesPerTier" -> 1,
  "CacheDir" -> FileNameJoin[{$ProjectRoot, "workspace", "mma-Box-region-solve", "cache"}],
  "OutputDir" -> FileNameJoin[{$ProjectRoot, "workspace", "mma-Box-region-solve"}],
  "LogDir" -> FileNameJoin[{$ProjectRoot, "workspace", "mma-Box-region-solve", "cache"}],
...
result = ScheduledRegionSolve["Box", familyConfig, scheduleConfig];
...
ibpFile = FileNameJoin[{$ProjectRoot, "workspace", "mma-Box-region-solve", "IBPMat_Box.bin"}];
ringFile = FileNameJoin[{$ProjectRoot, "workspace", "mma-Box-region-solve", "RingData_Box.bin"}];
Print["\nOutput:"];
