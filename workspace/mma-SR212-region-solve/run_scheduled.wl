(* run_scheduled.wl -- MMA ScheduledRegionSolve for SR212 (5-props, 26 sectors) *)
...
Print["========================================"];
Print["ScheduledRegionSolve SR212 (MMA)"];
...
scheduleConfig = <|
  ...
  "MaxRetriesPerTier" -> 1,
  "CacheDir" -> FileNameJoin[{$ProjectRoot, "workspace", "mma-SR212-region-solve", "cache"}],
  "OutputDir" -> FileNameJoin[{$ProjectRoot, "workspace", "mma-SR212-region-solve"}],
  "LogDir" -> FileNameJoin[{$ProjectRoot, "workspace", "mma-SR212-region-solve", "cache"}],
...
result = ScheduledRegionSolve["SR212", familyConfig, scheduleConfig];
...
ibpFile = FileNameJoin[{$ProjectRoot, "workspace", "mma-SR212-region-solve", "IBPMat_SR212.bin"}];
ringFile = FileNameJoin[{$ProjectRoot, "workspace", "mma-SR212-region-solve", "RingData_SR212.bin"}];
Print["\nOutput:"];
