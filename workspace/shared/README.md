...
| `docs/Test-RingData.md` | RingData binary verification + pitfalls |
...
## RingData Verification (C++ vs MMA binary)

Separate from the Relation modules above: `verify_ring_builder` compares C++ `RingData_*.bin` output against MMA reference binaries at byte level. Each cell represents a prime component of a sector with its Groebner basis → monomial basis → A/Ainv matrices.

```bash
cd /home/ykm/Large-Index-Expansion-CPP
./build/verify_ring_builder              # bub00, Tri, bub10, bub11
./build/verify_ring_builder TB123        # single family
```

**Key rules for modifying `verify_ring_builder.cpp`:**
- Cell matching must use multimap (not single-value map) — sectors can have multiple prime components
- Sort comparator needs `nb` tiebreaker — same sector + same weight cells are otherwise non-deterministic
- Always regenerate stale reference files after pipeline changes (e.g., `groebner(I)` → `std(I)`)

For full details see **[docs/Test-RingData.md](docs/Test-RingData.md)**.
