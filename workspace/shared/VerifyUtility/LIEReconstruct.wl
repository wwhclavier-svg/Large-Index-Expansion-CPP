...
(* ::Title:: *)
(*LIEReconstruct - Full-featured Relation Reconstruction*)
...
BeginPackage["LIEReconstruct`"]

(* Main exported function - compatible with reconstructRelation5 interface *)
LIEReconstruct::usage = "LIEReconstruct[level, degree, order, hexpnlist, areglist, topsec, vlist, opts] \
...
CoefficientEquations[expr_,vars_]:=Module[{eqs,varsSym,exprConverted,convRule},
If[expr==={},Return[{}]];
(* \:5982\:679c vars \:662f\:5b57\:7b26\:4e32\:5217\:8868\:ff0c\:4e34\:65f6\:8f6c\:6362\:4e3a\:7b26\:53f7\:8fdb\:884c\:8ba1\:7b97 *)
If[StringQ[vars[[1]]],
  ...
  (* \:5426\:5219\:76f4\:63a5\:4f7f\:7528\:7b26\:53f7 *)
...
GenerateAnsatz[mode_, rank_, maxDeg_, limitSector_, nVars_, vlist_] := Module[
  ...
  levelFilter = Switch[mode,
    ...
    _, Total[vlist - List @@ #] &       (* Default: Pyramid *)
  ];
...
BuildVariableMatrices[aList_, aRule_, aIndep_, basisMatrix_, modulus_] := Module[
  ...
  varAMatrix = Association@Table[
    j -> With[{rules = MonomialRulesPower[aList[[j]] /. aRule, aIndep]},
      FFReduce[Values[rules] . (Keys[rules] /. basisMatrix), modulus]
    ],
    {j, Length[aList]}
  ];
...
ComputeBasisPower[alphaList_, areg_, modulus_:0] := Module[
  {aRule, aIndep, aDep, aList, basisMatrix, basis, nb, 
   varAMatrix, varBMatrix, indexList, powerTable},
  ...
  (* Convert alpha list to index list *)
  indexList = # & /@ alphaList;
  
  (* Build variable matrices - same for rational and algebraic cases *)
...
*)
  ...
  If[eqs=!={},Print["eqs:  ",MatrixForm@RowReduce@Join[#[[2]],List/@#[[1]],2]&@CoefficientArrays[eqs,bVarReg]]];
...
(* ::Section:: *)
(*Main Function: LIEReconstruct*)


LIEReconstruct[rankLevel_, degree_, order_, hexpnList_, aregList_, topsec_, vlistIn_, opts:OptionsPattern[]] := Module[
  {
    (* Options *)
  ...
  Print["Level = ",rankLevel,"  Degree = ",degree];
