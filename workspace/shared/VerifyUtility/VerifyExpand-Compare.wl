...
For[k = 0, k <= maxk, k++,
    ...
    diffFlat = If[ListQ[diff], Total[Abs[diff], Infinity], diff];
...
moduleDetailOrders[] := (
    ...
    For[k = 0, k <= maxDetail, k++,
        ...
        dFlat = If[ListQ[d], Total[Abs[d], Infinity], d];
        status = If[dFlat === 0, "[MATCH]", "[MISMATCH]"];
...
moduleSummaryTable[] := (
    ...
    Do[
        ...
        d = PolynomialMod[Expand[mmaP - cppP], modulus];
        dFlat = If[ListQ[d], Total[Abs[d], Infinity], d];
        result = If[dFlat === 0, "MATCH", "MISMATCH"];
        If[dFlat =!= 0, allMatch = False];
