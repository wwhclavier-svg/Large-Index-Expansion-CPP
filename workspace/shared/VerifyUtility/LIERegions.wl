...
regionsBySectors[ibpeqslim_, sectorlist_, Alist_, vlist_, OptionsPattern[]] := 
 Module[{i, sec, ne, ibpeqs = ibpeqslim /. FromCharacterCode[{110}] -> 0, ibpeqssub, 
   expregsub, expregdata, ibpmatsub, recmatsub, hmatsub, hnulsub, 
   char = OptionValue[Modulus], regAssoc, secStart, regTime, buildTime, 
   result, nRegs, secElapsed, sectorResult},
  ...
  expregdata = Association @ Table[
      ...
      {regTime, expregsub} = AbsoluteTiming @ TimeConstrained[
        expRegSolve2[ibpeqssub, Alist, vlist, "LimitSector" -> sec, Modulus -> char, Verbose -> OptionValue[Verbose], "Timeout" -> OptionValue["Timeout"]],
        OptionValue["Timeout"],
        $Failed
      ];
      sectorResult = If[expregsub === $Failed,
        If[OptionValue["TimeoutMarker"] =!= None,
          sec -> OptionValue["TimeoutMarker"]
          ,
