...
(* Load SingularCoordinateRing for singularBivarPrimeInfo *)
If[!FileExistsQ["SingularCoordinateRing.wl"],
  Print["Warning: SingularCoordinateRing.wl not found. Falling back to MMA bivarPrimeInfo."];
];
If[!ValueQ[SingularCoordinateRing`singularBivarPrimeInfo],
...
Options[expRegSolve2]={Modulus->0,"LimitSector"->{},"Parameters"->{},"ZeroDim"->True,Verbose->False,"Timeout"->Infinity};
expRegSolve2[ibpeqs_,Alist_,vlist_,OptionsPattern[]]:=Module[
  ...
  time=AbsoluteTiming[
    agb=If[char>0,
      SingularGroebnerBasisDefault[aeqs,Join@@avar,Modulus->char,"MonomialOrder"->"lp","SingularTimeout"->If[OptionValue["Timeout"]=!=Infinity,OptionValue["Timeout"],0]],
      GroebnerBasis[aeqs,Join@@avar,Modulus->char]
    ]
  ][[1]];
  If[!ListQ[agb], PrintF["ERROR: SingularGroebnerBasisDefault returned non-List"]; Return[$Failed]];
  ...
  time=AbsoluteTiming[aprimelist=SingularMinAssPrime[agbGlobal,Join@@avarGlobal,Modulus->char,"MonomialOrder"->"lp","SingularTimeout"->If[OptionValue["Timeout"]=!=Infinity,OptionValue["Timeout"],0]]][[1]];
  If[!ListQ[aprimelist], PrintF["ERROR: SingularMinAssPrime failed"]; Return[$Failed]];
  ...
  If[!ListQ[aprimelist], PrintF["ERROR: SingularMinAssPrime output parse failed"]; Return[$Failed]];
  ...
  aprimelist=If[char>0,
    SingularGroebnerBasisDefault[#,Join@@avar,Modulus->char,"MonomialOrder"->"lp","SingularTimeout"->If[OptionValue["Timeout"]=!=Infinity,OptionValue["Timeout"],0]],
    GroebnerBasis[#,Join@@avar,Modulus->char]
  ]&/@aprimelist;
  res=If[Length[DownValues[SingularCoordinateRing`singularBivarPrimeInfo]] > 0,
    SingularCoordinateRing`singularBivarPrimeInfo[aprimelist,avar[[2]],avar[[1]],"FractionSymbol"->"B",Modulus->char,"Parameters"->avar[[3]],"LimitSector"->limitSector,Verbose->OptionValue[Verbose]],
    bivarPrimeInfo[aprimelist,avar[[2]],avar[[1]],"FractionSymbol"->"B",Modulus->char,"Parameters"->avar[[3]],Verbose->OptionValue[Verbose]]
  ];
