(* run_scheduled.wl -- Run ScheduledRegionSolve for Penta1L-5m *)
(* Usage: wolframscript -file workspace/C023-Penta1L-5m-mma-region-solve/run_scheduled.wl *)
...
Print["========================================"];
Print["ScheduledRegionSolve Penta1L-5m"];
...
Get[$FamilyDBPath];
If[!KeyExistsQ[$FamilyDatabase, "Penta1L-5m"],
  Print["ERROR: Penta1L-5m not found in FamilyDatabase"];
  Exit[1]
];
config = $FamilyDatabase["Penta1L-5m"];
...
scheduleConfig = <|
  ...
  "MaxRetriesPerTier" -> 1,
  "CacheDir" -> FileNameJoin[{$ProjectRoot, "workspace", "C023-Penta1L-5m-mma-region-solve", "cache"}],
  "OutputDir" -> $ProjectRoot,
  "LogDir" -> FileNameJoin[{$ProjectRoot, "workspace", "C023-Penta1L-5m-mma-region-solve", "cache"}],
...
result = ScheduledRegionSolve["Penta1L-5m", familyConfig, scheduleConfig];
...
ibpFile = FileNameJoin[{$ProjectRoot, "IBPMat_Penta1L-5m.bin"}];
ringFile = FileNameJoin[{$ProjectRoot, "RingData_Penta1L-5m.bin"}];
Print["\nOutput files:"];
...
Print["\n========================================"];
Print["ScheduledRegionSolve Penta1L-5m complete."];
Print["========================================"];
